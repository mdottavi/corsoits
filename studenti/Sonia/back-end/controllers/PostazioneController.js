...
class PostazioneController {
    ...
    static async listPostazione (req, res) {
        let luogo = null;
        let disponibili = false;
        let pagnum = 1;
        if (req.query.pag) pagnum = req.query.pag;
        if (req.query.luogo) luogo = req.query.luogo
        logger.debug ('LIBERO:' + req.query.libero);
